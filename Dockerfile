...
RUN apk add --no-cache nfs-utils && \
    mkdir -p /exports && \
    echo "/exports *(rw,sync,no_subtree_check,no_auth_nlm,insecure,all_squash,anonuid=65534,anongid=65534)" > /etc/exports
WORKDIR /exports

ENTRYPOINT ["/bin/sh", "-c", "rpcbind && rpc.nfsd && rpc.mountd -F"]
